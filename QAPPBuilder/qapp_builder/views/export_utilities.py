...
def set_fake_header_style(paragraph, format_type='docx'):
    if format_type == 'docx':
...
def create_toc(doc_or_elements, c, format_type):
    """Set up the Table of Contents page."""
    if format_type == 'docx':
        paragraph = doc_or_elements.add_paragraph()
        run = paragraph.add_run()
        ...
        doc_or_elements.append(Paragraph("Table of Contents", heading_style))
        ...
        for entry in toc_entries:
            doc_or_elements.append(Paragraph(entry, normal_style))
            doc_or_elements.append(Spacer(1, 12))


def add_heading(doc_or_elements, c, text, level, format_type):
    if format_type == 'docx':
        doc_or_elements.add_heading(text, level=level)
    elif format_type == 'pdf':
        from reportlab.platypus import Paragraph, Spacer
        from reportlab.lib.styles import getSampleStyleSheet
        styles = getSampleStyleSheet()
        heading_style = styles[f'Heading{level}']
        doc_or_elements.append(Paragraph(text, heading_style))
        doc_or_elements.append(Spacer(1, 12))


def add_paragraph(doc_or_elements, c, text, format_type, name='Calibri (Body)',
                  size=11, bold=False, italic=False,
                  alignment=WD_ALIGN_PARAGRAPH.LEFT):
    if format_type == 'docx':
        p = doc_or_elements.add_paragraph()
        run = p.add_run(text)
        set_font(run, name, size, bold, italic, format_type=format_type)
        p.alignment = alignment
    elif format_type == 'pdf':
        from reportlab.platypus import Paragraph, Spacer
        style = set_font(
            None, name, size, bold, italic, format_type=format_type)
        doc_or_elements.append(Paragraph(text, style))
        doc_or_elements.append(Spacer(1, 12))


def add_table(doc_or_elements, c, data, format_type, column_widths=None):
    if format_type == 'docx':
        table = doc_or_elements.add_table(rows=len(data), cols=len(data[0]))
